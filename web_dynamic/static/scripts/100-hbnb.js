document.addEventListener('DOMContentLoaded', () => {
    ...
    const checked_cities = {};
    const list = {'amenities': checked_amenities, 'states': checked_states, 'cities': checked_cities};
    const checkbox = document.querySelectorAll('input[type=checkbox]').forEach(function (checks) {
        checks.addEventListener("change", function () {
            if (this.checked) {
                list[this.id][checks.getAttribute('data-id')] = checks.getAttribute('data-name');
            }
            else {
                delete list[this.id][checks.getAttribute('data-id')];
            }
            const all_states_cities = [...Object.values(checked_states), ...Object.values(checked_cities)];
            document.querySelector(".locations h4").textContent = all_states_cities.join(', ');
            document.querySelector(".amenities h4").textContent = Object.values(checked_amenities).join(', ');
        });
    });
    ...
    function load_places(data) {
        ...
        if (section_place.hasChildNodes()) {
            while (section_place.firstChild) {
                section_place.removeChild(section_place.firstChild);
            }
        }
    ...
    button.addEventListener("click", function () {
        let request_data = {
            amenities: Object.keys(checked_amenities),
            states: Object.keys(checked_states),
            cities: Object.keys(checked_cities)
        }
        fetch("http://localhost:5001/api/v1/places_search", {
            method: 'POST',
            headers: {
